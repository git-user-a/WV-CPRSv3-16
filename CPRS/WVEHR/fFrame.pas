unit fFrame;
{ This is the main form for the CPRS GUI.
  It provides a patient-encounter-user framework
...
uses
  ...
  Tabs, ComCtrls, ExtCtrls, Menus, StdCtrls, Buttons, OleCtrls, RichEdit,
  ORFn, ORCtrls, Trpcb, VA508AccessibilityManager,
  fBase508Form, fPage, uConst,
{$IFDEF CCOW}
...
{$ENDIF}
  ...
  public
    ...
    procedure PatientSelect(aSender: TObject;Default: String = '');
...
{$ELSE}
  fPtSel, fPtSelOptns,
{$ENDIF}
  fCover, fProbs, fMeds, fOrders, rOrders,
  fNotes, fConsults, fDCSumm,
  rMisc, Clipbrd, fLabs, fReports, rReports, fPtDemo, fEncnt, fPtCWAD, uCore,
  fAbout, fReview,
  uGN_RPCLog,
  fxLists,
  ORSystem, fRptBox,
//  fSplash,
  rODAllergy, uInit, fLabTests, fLabInfo,
  uReminders, fReminderTree, ORClasses, fDeviceSelect, fDrawers,
  fReminderDialog, ShellAPI, rVitals,
  fOptions, fGraphs, fGraphData, rTemplates, fSurgery, rSurgery, uEventHooks,
  uSignItems,
  fDefaultEvent, rECS, fIconLegend, uOrders,
  DateUtils, uSpell,
  uOrPtf, fPatientFlagMulti,
  fAlertForward, UBAGlobals, fBAOptionsDiagnoses, UBACore, fOrdersSign, uVitals,
  fOrdersRenew, fMHTest, uFormMonitor
{$IFDEF CCOWBROKER}
    , CCOW_const
{$ENDIF}
    , VA508AccessibilityRouter, fOtherSchedule, VAUtils,
  uVA508CPRSCompatibility, fIVRoutes,
  // eRx  9/4/12
...
begin
  ...
  if assigned(frmSurgery) then
    frmSurgery.ClearPtData;
  frmLabs.ClearPtData;
  frmGraphData.ClearPtData;
  frmReports.ClearPtData;
  tabPage.TabIndex := PageIDToTab(CT_NOPAGE);
  // to make sure DisplayPage gets called
...
begin
  with stsArea do
    FFixedStatusWidth := Panels[0].Width + Panels[2].Width + Panels[3].Width +
      Panels[4].Width;
end;
...
(*
  {$IFDEF DEBUG}
  if not IsCompatibleVersion(sWar) then
      if (MessageDLG(
        'THIS DEBUG BUILD IGNORES VERSION VALIDATION'+CRLF+
        'DO NOT USE IT IN PRODUCTION ACCOUNTS' + CRLF + CRLF + sWar + CRLF+CRLF +
        'Proceed?', mtWarning,[mbYes,mbNo],0) = mrNo) then
      begin
        Close;
        Exit;
      end;
...
begin
  ...
  aDFN := ParamSearch('DFN');
  if aDFN <> '' then
    PatientSelect(nil,aDFN)
  else
    mnuFileOpenClick(Self);
...
{$ENDIF}    //WV End Change to patient age
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((Sender = mnuFileOpen)
{$IFDEF CCOW}
      or (AllowContextChangeAll(Reason))
{$ENDIF}
      )
...
begin
  ...
  if (Patient.DFN = '') or (Sender = mnuFileOpen) or (Sender = mnuFileNext) or
    (Sender = mnuViewDemo) then
    aDefault := '';
...
{$ELSE}
          if (Patient.DFN = '') or (aSender = mnuFileOpen) or
            (aSender = mnuFileNext) or (aSender = mnuViewDemo) then
            begin
...
procedure TfrmFrame.CallERx(Action: Integer);
