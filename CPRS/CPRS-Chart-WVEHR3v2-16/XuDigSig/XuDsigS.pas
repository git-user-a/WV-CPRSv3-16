...
// AA             pce := CertFindExtension(PChar('2.5.29.31'), cExtension, rgExtension);
              pce := CertFindExtension(LPCSTR('2.5.29.31'), cExtension, rgExtension);
              if pce <> nil then
...
// AA       PChar(szReaders),
    ...
    while not done do
    begin
// AA     dwi := SCardConnectA(fhSC, PChar(Str), SCARD_SHARE_SHARED, 3, fhCard, @ActiveProtocol);
      dwi := SCardConnectA(fhSC, LPStr(Str), SCARD_SHARE_SHARED, 3, fhCard, @ActiveProtocol);
      if dwi = SCARD_S_SUCCESS then
        done := true
      else
      begin
