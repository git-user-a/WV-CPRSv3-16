...
// VWPT ADDITIONS FOR ENHANCED PATIENT LOOKUP
procedure ListPtByOther(Dest: TStrings; const othertext: string;
  caption: string); // :string; radbutton:Tobject);
var
  i: Integer;
  tmp, tmp1: string;
  tmpList: TStrings;
begin
  ...
  try
    ...
    begin
      tmp1 := Piece(RPCBrokerV.Results[i], U, 3);
      tmp1 := Piece(tmp1, #9, 1);
      tmp := Pieces(RPCBrokerV.Results[i], U, 1, 2) + U + tmp1 + U +
        Pieces(RPCBrokerV.Results[i], U, 4, 5);
      tmpList.Add(tmp);
    end;
    Dest.Assign(tmpList);
  finally
...
begin
  ...
  begin
    ...
    SSN  := Piece(x, U, 1);
    DOB  := Piece(x, U, 2);
    //agp WV change strip time off for patient selection string
    if pos('.', DOB) > -1 then DOB := Piece(DOB, '.', 1);
