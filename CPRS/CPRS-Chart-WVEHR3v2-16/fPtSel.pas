...
begin
 Result := True;
 if (RadioGroup1.itemindex = -1) or (RadioGroup1.itemindex = 0) or
   (itimson = 0) then
   Result := False;
end;
...
procedure TfrmPtSel.cboPatientChange(Sender: TObject);
    ...
    // AA begin
  //function getLineNumber(aLine:String):Integer;
//  procedure getLineNumber(aLine: String; var lineNum: integer);
//  var
//     i1: integer;
//     s1: string;
//  begin
////    Result := -1;
//    lineNum := -1;
//    for i1 := 0 to cboPatient.Items.Count - 1 do
//      begin
//        s1 := cboPatient.Items[i1];
//        if pos(aLine,s1)= 1 then
//          begin
////            Result := i;
//            lineNum := i1;
//            break;
...
var
 index, i1, j: Integer;
  s, ss, sTarget, tmp: String;
  slData, sltemp: TStringList;

begin
  with cboPatient do
    if IsOther(Integer(1)) and (IsRPL <> '1') and (enhanceskip = 0) and
      (frmPtSelOptns.IsEnhanced(Text)) and
      (not frmPtSelOptns.IsPatientName(Text)) then
    begin
      if (IsRPL = '1') then
      ...
      begin
        ...
        ListPtByOther(Items, Text, Caption);
        // one extra argument on radio button selection
        if Items.Count > 0 then
        begin
          // itimson :=0 ;//no check for timson change in cbopatient
          // until after click event finished
    ...
    else // agp wv change removed if statement
      ...
      else if (not IsOther(Integer(1))) and (IsRPL <> '1') and (enhanceskip = 0)
        and (frmPtSelOptns.IsEnhanced(Text)) and
        (not frmPtSelOptns.IsPatientName(Text)) then
      begin
        // AA begin
        ...
        // j := getLineNumber(LLS_LINE, lineNum);
        j := -1;
        for i1 := 0 to cboPatient.Items.Count - 1 do
        begin
          tmp := cboPatient.Items[i1];
          if pos(LLS_LINE, tmp) = 1 then
          begin
            j := i1;
            break;
          end;
...
begin
  // vwpt enhanced   on click or double clck set mode back to normal to
  // a) not allow change event erroneously checked with false lookup,
  //    and
  // b) immedicately put back into normal mode without separate step needed.
...
begin
  ...
  begin
    itimson := 1;
    cboPatient.Text := '';
    if (frmPtSelOptns.radAll.Checked <> True) and
      (frmPtSelOptns.radDflt.Checked <> True) then
    begin
      radiogrp1index := 0;
      RadioGroup1.itemindex := 0;
      RadioGroup1.SetFocus;
