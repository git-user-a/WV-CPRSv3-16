...
uses
  ...
  uGN_RPCLog,
  fxLists,
  ORSystem, fRptBox, fSplash, rODAllergy, uInit, fLabTests, fLabInfo,
    ...
    begin
      ...
      begin
        ...
        if (Patient.DFN = '') or (Sender = mnuFileOpen) or (Sender = mnuFileNext) or (Sender = mnuViewDemo) then
          begin
{$IFDEF WORLDVISTA}
            if { WorldVistAPtSelect.Checked } True then
            begin
              if _SelectPatient <> mrOK then
              begin
                pnlPatient.Enabled := True;
//                if Patient.DFN <> '' then
//                  exit;
...
procedure TfrmFrame.CallERx(action: Integer);     //ERx 9/4/12  added entire function
var
...
{$ENDIF}
  ...
  try
    ...
    Exit;
      ...
      varUrl := Null;
      while (results.Count > 0) and not AnsiStartsText('RxInput=', results[0]) do
      begin
        line := Trim(results[0]);
        results.Delete(0);
