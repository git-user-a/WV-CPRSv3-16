...
begin
  if aFormat <> '' then
    ...
    Result := 'T'//OvcIntlSup.TrueChar
   else
     Result := 'F'//OvcIntlSup.FalseChar;
end;
...
begin
  ...
  else if AnsiStartsText(aValue, 'T'{OvcIntlSup.TrueChar})   then Result := True
  else if AnsiStartsText(aValue, 'Y'{OvcIntlSup.YesChar})    then Result := True
